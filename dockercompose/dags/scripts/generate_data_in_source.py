...
def get_source_conn():
    try:
        ...
        return conn
    # логирование и завершение процесса при ошибке подключения
    except Exception as e:
        logger.error(f'Failed to get source connection. Error: {str(e)}')
        raise


# генерация новых гостей
def generate_guest_data(conn):
    new_guests_number = random.randint(500, 1000)  # случайное количество новых гостей
    ...
    for _ in range(new_guests_number):
        ...
        date_of_birth = fake.date_of_birth(minimum_age=age_group[0], maximum_age=age_group[1])  # Дата рождения (18–99 лет)
...
def fill_initial_room_types():
    # типы номеров с начальными ценами
    room_types = {
        'standard': 100.00,
        'deluxe': 150.00,
        'suite': 250.00,
        'executive_suite': 400.00,
        'family_room': 200.00,
        'studio': 180.00,
        'city_view': 300.00,
        'mountain_view': 280.00
    }
    initial_records = []
...
def generate_bookings():
    ...
    num = random.randint(1000, 4000)  # количество новых бронирований
    ...
    with get_source_conn() as conn:
        with conn.cursor() as cursor:
            ...
            for i in range(num):
                ...
                expected_length_of_stay = random.randint(1, random.randint(4,8))  # случайное количество планируемых дней пребывания
...
def generate_stays_and_revenue():
...
def fill_initial_maintenance_types():
...
def generate_maintenance_facts():
    # случайное количество обслуживаний
    maintenance_count = random.randint(10, 40)
    with get_source_conn() as conn:
        with conn.cursor() as cursor:
...
def generate_maintenance_types_changes():
    with get_source_conn() as conn:
        with conn.cursor() as cursor:
            # время обновления записей
            update_time = datetime.now()
